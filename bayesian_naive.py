import matplotlib.pyplot as plt, numpy as np, tensorflow_probability as tfp
...
# USING BAYESIAN NAIVE MODELLING TO PLOT SLEEP STATES
def bayesian_naive(counts, length, alpha):
    for i in range(len(alpha)):
        alpha[i] += counts[i]
    theta_posterior = tfp.distributions.Dirichlet(alpha)
    print(theta_posterior.sample())
    state = []
    time = []
    current_time = 0  # zero since trial hasn't started
    for i in range(0, length - 1):
        current_time += 1  # Measures every 30 seconds
        state.append(np.random.choice([1,2,3,4,5], p=theta_posterior.sample()))
    ...
    font1 = {'family':'serif', 'color':'black'}
    plt.title("Bayesian Model (Assuming Independence Between Sleep States)")
...
def main():
    ...
    #bars(states, count)
    bayesian_naive(count, len(sequence), [1,1,1,1,1])
    return
